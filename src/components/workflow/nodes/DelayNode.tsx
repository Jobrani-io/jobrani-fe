...
import { Clock, X, UserPlus, Eye, MessageSquare, Heart, Send, GitBranch } from 'lucide-react';
...
const DelayNode: React.FC<DelayNodeProps> = ({ id, data }) => {
  ...
  const gradientColor = data.color || 'from-orange-400 to-amber-500';
  const borderColor = data.color ? 
    `border-${data.color.split('-')[1]}-200` : 'border-orange-200';
  const bgColor = data.color ? 
    `from-${data.color.split('-')[1]}-50 to-${data.color.split('-')[2]?.split('-')[0] || 'amber'}-50` : 
    'from-orange-50 to-amber-50';
  const handleColor = data.color ? 
    `!bg-${data.color.split('-')[1]}-500` : '!bg-orange-500';

  const IconComponent = (data.icon && iconMap[data.icon as keyof typeof iconMap]) || Clock;
